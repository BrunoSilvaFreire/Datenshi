...
namespace Datenshi.Scripts.Editor {
    ...
    public class MovableBodyEditor : UnityEditor.Editor {
        ...
        private void OnSceneGUI() {
            var points = serializedObject.FindProperty(nameof(MovableBody.Curve)).FindPropertyRelative(nameof(BezierCurve.Points));
            ...
            for (var i = 0; i < end; i++) {
                ...
                Handles.Label(startPos + Vector3.up * 2, $"Point #{i}", s.FindStyle(GUISkinProperties.INGameObjectHeader));
                DrawHandles(a);
                if (i >= end - 1) {
                    continue;
                }
        ...
        private void DrawBezier(SerializedProperty a, SerializedProperty b) {
            ...
            Handles.DrawBezier(startPos, endPos, startPos + startTangent, endPos + endTangent, Color.green, Texture2D.whiteTexture, curveWidth);
        }
        ...
        private void DrawHandles(SerializedProperty point) {
            ...
            inTangent.vector3Value = Handles.PositionHandle(pointPos + inTangent.vector3Value, Quaternion.identity) - pointPos;
            var outTangent = point.FindPropertyRelative(nameof(BezierPoint.EndTangent));
            outTangent.vector3Value = Handles.PositionHandle(pointPos + outTangent.vector3Value, Quaternion.identity) - pointPos;
