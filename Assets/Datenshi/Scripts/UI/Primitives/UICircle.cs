...
namespace UnityEngine.UI.Extensions {
    ...
    public class UICircle : UIPrimitiveBase {
        ...
        protected override void OnPopulateMesh(VertexHelper vh) {
            ...
            float Diameter = Mathf.Min(rect.width, rect.height) - Padding; //correct for padding and always fit RectTransform
            float outerDiameter = -rectTransform.pivot.x * Diameter;
            //float outerDiameter = Mathf.Min(rect.width, rect.height);
            float innerDiameter = -rectTransform.pivot.x * Diameter + Thickness;
            //float innerDiameter = outerDiameter + Thickness;
            vh.Clear();
            indices.Clear();
            vertices.Clear();
            ...
            if (Fill)
                iV = Vector2.zero; //center vertex to pivot
            vertex.position = iV;
